...
- SVG: `rect`, `circle`, `ellipse`, `line`, `polygon`, `polyline`, linear/quadratic/cubic/arc `path`, simple rect/text, line/text, rect-background/line-border grids, and simple `foreignObject` HTML tables with basic `colspan`/`rowspan` merges as native DrawingML tables, `text`, simple `tspan`, link/group containers, and basic `switch` fallback selection
